...
public class JWTAuthorizationFilter extends BasicAuthenticationFilter {
	// this is a middle were to check if the request have a valid jwt token or not
	...
	protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
		...
		String tokenOriginal = header.replace("Bearer ", "");
		Jws<Claims> claims = Jwts.parser().requireSubject("JwtToken").setSigningKey(SECRET.getBytes())
				.parseClaimsJws(tokenOriginal);

		Date expiryDate = claims.getBody().get("exp", Date.class);
		Date now = new Date();
		if (expiryDate.before(now)) {
			throw new FailedException("token has been expired");
		}
